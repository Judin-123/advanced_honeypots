"""
ML-Powered Honeypot Main Application
Core application that orchestrates ML analysis and dynamic adaptation
"""
...
from src.monitoring.dashboard import HoneypotDashboard
...
class MLHoneypot:
    """Main application class for ML-powered honeypot"""
    ...
    def __init__(self, config_path: str = "configs/config.yaml"):
        ...
        self.dashboard = HoneypotDashboard(self.config)
    ...
    def start(self):
        ...
        logger.info("Starting ML-Powered Honeypot System")
    ...
    def _start_dashboard(self):
        ...
        try:
            port = self.config.get("monitoring", {}).get("dashboard_port", 5000)
            self.dashboard.run(host="0.0.0.0", port=port, debug=False)
        except Exception as e:
    ...
    def _perform_adaptation(self):
        ...
        try:
            ...
            if result.get('status') == 'adapted':
                ...
                self._send_adaptation_alert(result)
            
        except Exception as e:
            logger.error(f"Failed to perform adaptation: {e}")
    
    def _send_adaptation_alert(self, adaptation_result: Dict[str, Any]):
        """Send alert about adaptation (email, webhook, etc.)"""
        alert_email = self.config.get("monitoring", {}).get("alert_email")
        
        if alert_email:
            # In a real implementation, you'd send an email here
            logger.info(f"Would send adaptation alert to {alert_email}: {adaptation_result}")
